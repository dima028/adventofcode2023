...
gamesSplit = games.split("\n")

sum = 0

gamesSplit.each_with_index do |game, index|
  next if index == 0 || index == 101
  ...
  gameSplit.each do |minigame|
    next if minigame[0] == 'G' # skipping the "Game n" keys

    cubes = minigame.split(",")
    cubes.each do |cube|
      number, colour = cube.strip.split(" ")
      if colour == "red"
        maxRed = number.to_i if number.to_i > maxRed
      elsif colour == "blue"
        maxBlue = number.to_i if number.to_i > maxBlue
      elsif colour == "green"
        maxGreen = number.to_i if number.to_i > maxGreen
      end
    end
  end
  # print "red: #{maxRed}, blue: #{maxBlue}, green: #{maxGreen}\n"
