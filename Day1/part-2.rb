...
input.each do |line|
  ...
  # sanitize input
  nums.each do |num|
    indices = (0 ... line.length).find_all { |i| line[i, num.length] == num }
    line = line.gsub(num, n.to_s)
    
    # manually fix all oneight = 1eight errors, smaller number is parsed first
    line = line.gsub("1ight", "18")
    line = line.gsub("eigh2", "82")
    line = line.gsub("eigh3", "83")
    line = line.gsub("7ine", "79")
    line = line.gsub("5ight", "58")
    line = line.gsub("nin8", "98")
    line = line.gsub("tw1", "21")
    n+=1
  end
  n = 1
  ...
  presum = first.to_i * 10 + last.to_i
  puts presum
  sum += presum
  first = ''
  last = ''
