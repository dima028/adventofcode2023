pipes = ".|-J--7F7F-F|-F777F7J-J7F--7..-LJ7F|.F7L-77--F7|7.F-7-77.F.L---J.F---FLJF-F|77-|-----|-FL-J-|7-7FF.7--F7F7.F--F7.FF-F--J-.L-L77--77|---L7--7
...
def getS(pipes)
  index = pipes.index("S")
  offset = ( index / (pipes.lines.size + 1) ).floor()
  adj_id = index - offset
  y = (adj_id / pipes.lines.first.size).floor() + 1
  x = adj_id % (pipes.lines.first.size - 1)
  # print "1ST: ", pipes.lines[y][x], " x: ", x, " y: ", y, "\n"
  print " index: ", index, " adj: ", adj_id, " x: ", x, " y: ", y, "\n"
  $hm[[x,y]] = "."
  [ x, y ]
end
...
def getPipeStart(pipes, y_idx, x_idx)
    n = 1

    up = pipes.lines[y_idx - 1][x_idx]
    down = pipes.lines[y_idx + 1][x_idx]
    right = pipes.lines[y_idx][x_idx+1]
    left = pipes.lines[y_idx][x_idx-1]

    x1, y1, x2, y2, dir1, dir2 = nil
    print "1st: ", pipes.lines[y_idx][x_idx], "\n"

    # up: |, F, 7
    if y_idx >= 1 && up == "|" || up == "F" || up == "7"
        # n = followPipe(pipes, y_idx - 1, x_idx, "up", n)
        if x1 == nil && y1 == nil
          x1 = x_idx
          y1 = y_idx - 1
          dir1 = "up"
    ...
    # down: |, J, L
    if y_idx < (pipes.lines.count - 1) && down == "|" || down == "J" || down == "L"
        # n = followPipe(pipes, y_idx + 1, x_idx, "down", n)
        if x1 == nil && y1 == nil
          x1 = x_idx
          y1 = y_idx + 1
          dir1 = "down"
    ...
    # right: -, J, 7
    if x_idx < (pipes.lines.first.size - 1) && right == "-" || right == "J" || right == "7"
        # n = followPipe(pipes, y_idx, x_idx + 1, "right", n)
        if x1 == nil && y1 == nil
          x1 = x_idx + 1
          y1 = y_idx
          dir1 = "right"
    ...
    # left: -, F, L
    if x_idx >= 1 && left == "-" || left == "F" || left == "L"
        # n = followPipe(pipes, y_idx, x_idx - 1, "left", n)
        if x1 == nil && y1 == nil
          x1 = x_idx - 1
          y1 = y_idx
          dir1 = "left"
...
def twoDir(pipes, x1, y1, dir1, x2, y2, dir2, n)
  # puts x1, y1, dir1, x2, y2, dir2
  while x1 != x2 || y1 != y2
    # print " N : ", n, "\n"
    n += 2
    # print "(", y1, ", ", x1, "), sym: ", pipes.lines[y1][x1], "\n"
    # print "                 (", y2, ", ", x2, "), sym: ", pipes.lines[y2][x2], "\n"

    x1, y1, dir1 = followPipe(y1, x1, dir1)
    x2, y2, dir2 = followPipe(y2, x2, dir2)
  end
  $hm[[x1,y1]] = "."
  n += 1if x1 == x2 && y1 == y2
  return n
...
def oneDir(x, y, dir)
  n = 1
  while PIPES.lines[y][x] != "S"
    # print " N : ", n, "\n"
    n += 2
    x, y, dir = followPipe(y, x, dir)
    # x2, y2, dir2 = followPipe(pipes, y2, x2, dir2)
  end
  n += 1
  return n
end

def followPipe(y_idx, x_idx, dir)
    pipes = PIPES
    # print "NOW: ", pipes.lines[y_idx][x_idx], " x: ", x_idx, " y: ", y_idx, "\n"

    y = y_idx
    x = x_idx
    curr = pipes.lines[y][x]
    dir = dir
...
x1, y1, dir1, x2, y2, dir2 = getPipeStart(PIPES, y, x)
# n = oneDir(x1, y1, dir1)
n = twoDir(PIPES, x1, y1, dir1, x2, y2, dir2, 1)
area = 0

# puts $hm
PIPES.each_line.with_index do |line, yidk|
  prev = ""
  flag = false
  line.each_char.with_index do |char, xidk|
    curr = PIPES.lines[yidk][xidk]
    if $hm.key?([xidk, yidk])
      # print prev, " & ", curr, " : (", xidk, ",", yidk, "), ", flag, "\n" if curr != "|" && curr != "."

      if (curr == "|") || (curr == "S" && $s_shape == 2) ||
          (prev == "L" && curr == "7") || (prev == "F" && curr == "J") || (prev == "S" && curr == "J")
        flag = (flag == true) ? false : true
        print prev, " & ", curr, " : (", xidk, ",", yidk, "), ", flag, "\n" if curr != "|"
      end
    end
    if !$hm.key?([xidk, yidk]) && flag == true && xidk < PIPES.lines.first.size - 1 && yidk > 0 && yidk < PIPES.lines.size - 1
      # print PIPES.lines[yidk][xidk], " : (", xidk, ",", yidk, ").\n"
      area += 1
    end
    prev = curr if curr != "-" && curr != "|" && curr != "."
  end
end
