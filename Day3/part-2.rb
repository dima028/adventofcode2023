...
engine.each_line.with_index do |line, index|
  ...
  line.each_char.with_index do |char, idx|
    ...
    unless topRow.nil? && topAdjacent == false
      # puts topRow[idx]
      if is_integer?(char) 
        if symbols.include?(topRow[idx-1])
          xy = [idx-1, index-1]
          topAdjacent = true
        elsif symbols.include?(topRow[idx])
          xy = [idx, index-1]
          topAdjacent = true
        elsif symbols.include?(topRow[idx+1])
          xy = [idx+1, index-1]
          topAdjacent = true
        end
      end
    ...
    unless botRow.nil? && botAdjacent == false
      # puts botRow[idx]
      if is_integer?(char) 
        if symbols.include?(botRow[idx-1])
          xy = [idx-1, index+1]
          botAdjacent = true
        elsif symbols.include?(botRow[idx])
          xy = [idx, index+1]
          botAdjacent = true
        elsif symbols.include?(botRow[idx+1])
          xy = [idx+1, index+1]
          botAdjacent = true
        end
      end
