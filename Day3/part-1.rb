...
engine = "........440...............418..643.....438......740.261......................................727...........................870..............
...
engine.each_line.with_index do |line, index|
  # initializing flags to be used for adjacent checks
  numFlag = false
  symFlag = false
  topAdjacent = false
  leftAdjacent = false
  ...
  # get top and bot rows
  topRow = engine.each_line.with_index.find { |l, ix| ix == index - 1 }[0] if index > 0
  botRow = engine.each_line.with_index.find { |l, ix| ix == index + 1 }[0] if index < engine.each_line.count - 1

  line.each_char.with_index do |char, idx|
    if is_integer?(char) == true
      progNum << char
    else
      finNum = progNum
      progNum = ""
    ...
    # # # # # # #    S A M E   A D J A C E N T   C H E C K S
    # update flags for adjacent checks
    ...
    # # # # # # #   T O P   A D J A C E N T   C H E C K S
    unless topRow.nil? && topAdjacent == false
      # puts topRow[idx]
      if is_integer?(char) && (symbols.include?(topRow[idx-1]) || symbols.include?(topRow[idx]) || symbols.include?(topRow[idx+1]))
        topAdjacent = true
      end
    end
           
    # # # # # # #    B O T T O M    A D J A C E N T    C H E C K S
    unless botRow.nil? && botAdjacent == false
      # puts botRow[idx]
      if is_integer?(char) && (symbols.include?(botRow[idx-1]) || symbols.include?(botRow[idx]) || symbols.include?(botRow[idx+1]))
        botAdjacent = true
      end
    end
    
    # print "left: ", leftAdjacent, "  finNum: ", finNum, "\n" unless leftAdjacent == false
    if (leftAdjacent && finNum != "")
      print "finL: ", finNum, ", sum: ", sum, "\n"
      sum += finNum.to_i
      finNum = 0
      leftAdjacent = false
    end
    ...
    if finNum != ""
      if botAdjacent || topAdjacent
        print "fin: ", finNum, ", sum: ", sum, "\n"
        sum += finNum.to_i
        botAdjacent = false
        topAdjacent = false
      end
    ...
    if (rightAdjacent && finNum == "" && prevNum.to_i > 0)
      print "prev: ", prevNum, ", sum: ", sum, "\n"
      sum += prevNum.to_i
      prevNum = 0
      rightAdjacent = false
    end
